...
class ExerciseVariantGenerationIntegrationTest extends AbstractSpringIntegrationLocalCILocalVCTest {
    ...
    @Test
    @WithMockUser(username = EDITOR_LOGIN, roles = "EDITOR")
    void shouldRejectMilestoneAndUserStorySources() throws Exception {
        // Both are programming exercises by type, but only work inside their milestone group, so a clone would be broken.
        for (ProgrammingExercise source : List.of(new MilestoneExercise(), new UserStoryExercise())) {
            source.setTitle("Scrum source " + source.getClass().getSimpleName());
            source.setShortName("scrum" + source.getClass().getSimpleName().length() + TEST_PREFIX);
            source.setProgrammingLanguage(ProgrammingLanguage.JAVA);
            source.setCourse(course);
            source.setMaxPoints(10.0);
            source.generateAndSetProjectKey();
            Exercise saved = exerciseRepository.save(source);
            request.postWithResponseBody("/api/hyperion/exercises/" + saved.getId() + "/generate-variant", domainChangeRequest(standalonePlacement()), VariantJobStartDTO.class,
                    HttpStatus.BAD_REQUEST);
        }
    }
