...
export class CourseOverviewService {
    ...
    private groupCard(group: CourseExerciseGroup, members: Exercise[]): SidebarCardElement {
        ...
        return {
            ...
            startDate: dueDate,
            size: 'M',
            // The members mark group membership, so a search on a variant title still surfaces the group and the group
            // card stays highlighted while one of its variants is open. Only a milestone lists them as cards of their own.
            groupedItems: members.map((member) => this.mapExerciseToSidebarCardElement(member)),
            renderGroupedItems: group.type === 'milestone',
        };
    }
