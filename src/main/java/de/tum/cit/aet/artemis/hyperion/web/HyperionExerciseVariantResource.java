...
public class HyperionExerciseVariantResource {
    ...
    private void validateRequest(Exercise exercise, VariantGenerationRequestDTO request) {
        ...
        // Milestone and user story exercises are programming exercises by type, but they only work inside their milestone
        // group: they share its repositories, build plan and test case pool. A standalone clone would have none of that.
        if (exercise instanceof MilestoneExercise || exercise instanceof UserStoryExercise) {
            throw new BadRequestAlertException("Variant generation is not supported for milestone and user story exercises", ENTITY_NAME,
                    "variantGenerationNotSupportedForMilestone");
        }
