...
public class ParticipantScoreService {
    ...
    public List<ScoreDTO> calculateCourseScores(Course course) {
        ...
        Set<Exercise> exercisesToConsider = course.getExercises().stream().filter(Exercise::isCourseExercise)
                ...
                // A milestone group's user story points are carried by its MilestoneExercise, which is considered here in its
                // own right; the stories would count the same points a second time. See UserStoryExercise. The group's other
                // members are not part of that aggregate and count like any exercise.
                .filter(exercise -> !(exercise instanceof UserStoryExercise && exercise.getExerciseVariantGroup() instanceof MilestoneExerciseGroup)).collect(Collectors.toSet());
