...
<Callout variant={CalloutVariant.info}>
    ...
    Just make sure that you use unique port mappings for your Docker containers (e.g. `8081` for Jenkins, `8081` for Artemis).
</Callout>
...
jenkins:
    ...
    internal-urls:
        ci-url: http://jenkins:8081
        vcs-url: http://172.17.0.1:8081 # `http://host.docker.internal:8081` for Windows
    use-crumb: false
server:
    port: 8081
    url: http://172.17.0.1:8081 # `http://host.docker.internal:8081` for Windows
```
...
<Callout variant={CalloutVariant.info}>
  ...
  In case you use a different port than 80 (http) or 443 (https) for the communication, you have to append it to the both urls value, e.g. `127.0.0.1:8081`.
</Callout>
...
6. Run Jenkins by executing the following command (change the hostname and choose which port alternative you need)
   ...
   docker run -itd --name jenkins \
       ...
       -e VIRTUAL_HOST=your.jenkins.domain -e VIRTUAL_PORT=8081 \    # Alternative 1: If you are NOT using a separate NGINX instance
       -e LETSENCRYPT_HOST=your.jenkins.domain \                     # Only needed if Alternative 1 is used
       -p 8082:8081 \                                                # Alternative 2: If you ARE using a separate NGINX instance OR you ARE installing Jenkins on a local development computer
       -u root \
       jenkins/jenkins:lts
   ```
