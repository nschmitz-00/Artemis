...
**When accessing Artemis at `http://localhost:8081`** (direct Spring Boot, or `./gradlew bootRun`):
...
<Callout variant={CalloutVariant.info}>
    ...
    Artemis via port 8081 or port 9000. The Spring Boot server always runs on port 8081 — the Angular
...
| Allowed Redirects | `http://localhost:8081/...` and `http://localhost:9000/...` |
...
**Fix:** The preconfigured realm allows both `localhost:8081` and `localhost:9000`. If you changed the port, update the redirect URIs in the Keycloak admin console: **Realm: artemis → Clients → artemis → Valid Redirect URIs**.
