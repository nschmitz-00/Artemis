...
It auto-kills conflicting processes on ports 8081/9000, keeps services running between re-runs, and supports filtering individual tests.
...
The script automatically kills any processes occupying ports 8081 (server), 9000 (client), and 7921 (local VC SSH),
...
- **Host:** Spring Boot server via `./gradlew bootRun -x webapp` (port 8081)
- **Host:** Angular client via `pnpm start` (port 9000, proxies API calls to 8081)
